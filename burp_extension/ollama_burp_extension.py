...
class BurpExtender(IBurpExtender, IIntruderPayloadGeneratorFactory, ITab):
    ...
    _jTextURL = None
    _jURLButton = None
    aboutText = ("A Burp extension to interact with Ollama and use it as payload generator\n"
    ...
    _URL = "http://localhost:11434/api/"
    _URLText = "insert here the URL of your ollama instance"
    _fp = 1.0
    _mt = 50
    _pp = 0.0
    _temp = 1.0
    ...
    def registerExtenderCallbacks(self, callbacks):
        ...
        callbacks.setExtensionName("Ollama Burp Extension")
        ...
        self._GeneratorInstance = OllamaIntruderPayloadGenerator(self._prompt, self._message, self._model, self._fp,
                                                                 self._mt, self._pp, self._temp, self._tp, self._URL)
        return

    def getGeneratorName(self):
        return "Ollama Burp Extension"
    ...
    def initPanelConfig(self):
        ...
        self._jComboBoxModel = JComboBox(["Set the ollama URL"])
    ...
    def setAI(self, event=None):
        ...
        JOptionPane.showMessageDialog(None, "Ollama configured!")

    def setURL(self, event=None):
        self._URL = self._jTextURL.getText()
        self._GeneratorInstance.set_URL(self._URL)
        self._GeneratorInstance.reset()
        m = Requests.get(self._URL+"tags")
        self._jComboBoxModel.removeAllItems()
        for model in m["models"]:
            self._jComboBoxModel.addItem(model["name"])
        JOptionPane.showMessageDialog(None, "URL configured!")


class OllamaIntruderPayloadGenerator(IIntruderPayloadGenerator):
    def __init__(self, prompt, question, model, fp, mt, pp, temp, tp, URL):
        self._prompt = prompt
        self._OllamaModule = OllamaModule(prompt=prompt, model=model)
    ...
    def getNextPayload(self, baseValue):
        payload = self._OllamaModule.ask_ai()
        return payload
    ...
    def reset(self):
        print("-------------------------reset----------------------")
        return self._OllamaModule.reset(self._prompt, self._question, self._model, self._fp, self._mt, self._pp,
                                        self._temp, self._tp, self._URL)
